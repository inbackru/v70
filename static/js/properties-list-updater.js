...
// Track if SSR cards are present (initial load)
let ssrCardsPresent = null;
...
window.updatePropertiesList = function(properties) {
    ...
    // ✅ КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: Проверяем наличие SSR карточек при первом вызове
    if (ssrCardsPresent === null) {
        const existingCards = container.querySelectorAll('.property-card');
        ssrCardsPresent = existingCards.length > 0;
        console.log(`🔍 SSR cards check: Found ${existingCards.length} existing cards`);
        
        // Если есть SSR карточки и это server-side pagination, НЕ ОЧИЩАЕМ контейнер
        if (ssrCardsPresent && window.serverSidePagination) {
            console.log('✅ SSR cards detected with server-side pagination - PRESERVING existing cards');
            console.log('⏭️ Skipping updatePropertiesList to preserve SSR content');
            // Переинициализируем только функционал карточек, не трогая DOM
            reinitializeCardFeatures();
            return;
        }
    }

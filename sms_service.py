...
class RedSMSService:
    ...
    def __init__(self):
        self.login = os.environ.get('RED_SMS_LOGIN')
        self.api_key = os.environ.get('RED_SMS_API_KEY')
        
        if not self.login or not self.api_key:
            logger.warning("RED SMS credentials not fully configured")
            if not self.login:
                logger.warning("RED_SMS_LOGIN not found in environment variables")
            if not self.api_key:
                logger.warning("RED_SMS_API_KEY not found in environment variables")
    ...
    def send_verification_code(self, phone: str, code: str) -> Dict[str, Any]:
        ...
        message_text = f"Ваш код подтверждения InBack: {code}. Код действителен 10 минут."
